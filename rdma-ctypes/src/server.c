...
static void setup_client_resources(struct rdma_cm_id *cm_client_id) {
    ...
    int flags = fcntl(client_res->comp_channel->fd, F_GETFL);
    int rc = fcntl(client_res->comp_channel->fd, F_SETFL, flags | O_NONBLOCK);
    if (rc < 0) {
...
static void post_send_ACK() {
    struct ibv_wc wc;
    //server_buff.message = malloc(sizeof(struct msg));
    server_buff.message->type = HELLO;
    server_buff.message->data.offset = 111;
    ...
    while (rdma_get_cm_event(cm_event_channel, &received_event) == 0) {
        ...
        switch (cm_event.event) {
            ...
            case RDMA_CM_EVENT_ESTABLISHED:
                ...
                if (count >= 5) {
                    info("RDMA read returns empty data. Try again! \n");
                    break;
                }
            ...
            case RDMA_CM_EVENT_DISCONNECTED:
                ...
                disconnect_server(client_res, frame, cm_event_channel, cm_server_id);
                //sleep(2);
                info("Cleanup complete - starting over \n");
                break;
            default:
