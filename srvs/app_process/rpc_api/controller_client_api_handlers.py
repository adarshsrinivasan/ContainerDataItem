...
class ControllerClient(object):
    ...
    def TransferCDIs(self, process_id, transfer_id, transfer_mode, config):
        logging.info(f"TransferCDIs({self.host}:{self.server_port}): Sending request")
        proto_controller_cdi_configs = config.to_proto_controller_cdi_configs()
        message = pb2.TransferCDIsRequest(id=process_id, transfer_id=transfer_id, transfer_mode=transfer_mode,
                                          cdi_configs=proto_controller_cdi_configs)
