...
from srvs.combiner.rpc_api import controller_api_pb2_grpc as pb2_grpc, controller_api_pb2 as pb2
...
class ControllerClient(object):
    ...
    def RegisterProcess(self, process_id, name, namespace, node_ip, rpc_ip, rpc_port, uid, gid):
        ...
        #logging.info(f"RegisterProcess: process_id: {type(process_id)}, name: {type(name)}, namespace: {type(namespace)}, node_ip: {type(node_ip)}, rpc_ip: {type(rpc_ip)}, rpc_port: {type(rpc_port)}, uid: {type(uid)}, gid: {type(gid)}")
        message = pb2.RegisterProcessRequest(id=process_id, name=name, namespace=namespace,
                                             node_ip=node_ip, rpc_ip=rpc_ip, rpc_port=rpc_port, uid=str(uid), gid=str(gid))
